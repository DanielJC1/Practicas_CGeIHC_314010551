...
Window::Window(GLint windowWidth, GLint windowHeight)
{
	...
	articulacion3 = 0.0f;
	articulacion4 = 0.0f;
	articulacion5 = 0.0f; //Inicializar nuevas articulaciones
	articulacion6 = 0.0f;
	articulacion7 = 0.0f;
	articulacion8 = 0.0f;
	articulacion9 = 0.0f;
...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	if (key == GLFW_KEY_F)
	{
		if (theWindow->articulacion1 > -10.0) // Establecer límite de cierre del cofre
		{
		}
		else
		{ 
	...
	if (key == GLFW_KEY_G)
	{
		if (theWindow->articulacion1 < -80) //Establecer límite de apertura del cofre
		{
		}
		else
		{
	...
	if (key == GLFW_KEY_H)
	{
		theWindow->articulacion2 += 10.0; //Rota y traslada hacia adelante
	}
	if (key == GLFW_KEY_J)
	{
		theWindow->articulacion2 -= 10.0; //Rota y traslada hacia atrás
	}
	if (key == GLFW_KEY_K)
	{
